...
const FoodCard = ({ item }) => {
  ...
  const handleAddToCart = (itemData) => {
    ...
    if (user && user.email) {
      ...
      fetch("https://bistro-boss-server-sigma-ten.vercel.app/carts", {
        method: "POST",
        headers: {
        ...
        .then((data) => {
          if (data.insertedId) {
            refetch(); // refetch cart to update cart item in navbar
            Swal.fire({
              position: "top-end",
              icon: "success",
  ...
  return (
    <div className="card mx-auto card-compact w-[424px] text-center bg-[#F3F3F3] drop-shadow-xl shadow-xl">
      <figure>
        <img src={image} className="object-cover h-64 w-full" alt="Salad" />
      </figure>
